...
const columnIsFull = (column) => {
    ...
    for (let row = 0; row < boardModel.length; row++) {
        if (boardModel[row][column] != null) { //if the column contents is not null
            columnCountWithDisc++; //add 1 to our columnWith Disc
        }
    }
    if (columnCountWithDisc === 5) {
        draw++
        if (draw === 7) {
            genericDisplayMessage("It is a draw!");
            setTimeout(function () { alert("It is a draw!"), 5 })
            window.location = "index.html"
        }
    }
    if (columnCountWithDisc === boardModel.length) { //if the column with disc count matches the boardModel length the column is full
        return true;
    }
    return false;
};
...
const clickHandler = function (eventObj) {
    let gameboard = eventObj.currentTarget
    ...
    if (columnIsFull(columnAsNumber)) {
        ...
        if (winner != '') {
            ...
            setTimeout(function () { alert("The winner is " + winner), 5 })
            window.location = "index.html"
        } else {
            togglePlayer();
        }
...
const isGameOver = (board) => {
    var i, s, r = 'blackblackblackblack', y = 'redredredred';
    // rows
